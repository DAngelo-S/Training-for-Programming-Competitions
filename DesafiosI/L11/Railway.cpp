...
class Line {
    public:
        ...
        Point closerPointInTheSegFrom(Point other) {
            float a2, b2, m2, x, y;
            ...
            */
            ...
            if (aux.getDist(pa) <= aux.getDist(pb)) {
                return pa;
            }
            return pb;
        }
        ...
        bool isInSegment(Point p) {
            return (p.y >= min(pa.y, pb.y) && p.y <= max(pa.y, pb.y));
        }
...
int main () {
    ...
    while (cin >> x) {
        ...
        while (n<N) {
            ...
            aux = l.closerPointInTheSegFrom(M);
            D = M.getDist(aux);
